-- Recreate ingredients table as requested: all text columns per spec and add uuid id
...
begin
  if not exists (
    ...
    where schemaname='public' and tablename='ingredients' and policyname='ingredients_read'
  ) then
