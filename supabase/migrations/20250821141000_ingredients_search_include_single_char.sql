-- Re-enable single-character tokens and ensure whitespace is only a separator
-- AND semantics: every token must match at least one of the searchable columns
...
declare
  ...
  _tokens text[] := array(
    ...
    where btrim(t) <> ''  -- keep 1+ char tokens; drop only blanks
  );
  _where text := '';
  _cols text[] := array[
