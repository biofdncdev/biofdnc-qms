    ...
    if (!this.isViewer) {
      this.menus.push(
        ...
        ...(!this.isGivaudanAudit ? [
          ...
          { key: 'product', icon: 'inventory', label: 'Product', submenu: [
            { label: '품목 일괄 등록', path: '/app/product/update' },
            { label: '품목조회', path: '/app/product' },
            { label: '품목 조성비 등록', path: '/app/product/form' },
            { label: 'BOM TREE', path: '/app/product/bom-tree' },
            { label: '서류작성', path: '/app/product/docs' },
            { label: '서류양식', path: '/app/product/doc-templates', adminOnly: true }
          ] },
