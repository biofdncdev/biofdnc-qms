...
export class SupabaseService {
    ...
    try {
      if (isPdf){
        ...
        const { error, data } = await client.storage
          .from('rmd_records')
          .upload(recordsPath, file, { upsert: true, contentType: (file as any).type || undefined, cacheControl: '3600' });
        if (error){
  ...
  async listCompanies(){
    const client = this.ensureClient();
    try{
      const { data } = await client.from('companies').select('*').order('code', { ascending: true }) as any;
      return Array.isArray(data) ? data : [];
    }catch{
      // Fallback: derive company list from departments if companies table is unavailable
      try{
        const { data } = await client.from('departments').select('company_code,company_name') as any;
        const rows = Array.isArray(data) ? data : [];
        const map: Record<string, { id?: string; code: string; name: string | null }>= {};
        for (const r of rows){
          const code = (r as any)?.company_code; if (!code) continue;
  ...
  async upsertCompany(row: { id?: string; code: string; name: string }){
    ...
    if (!payload.id) payload.id = crypto.randomUUID();
    try{
      return await client.from('companies').upsert(payload, { onConflict: 'id' }).select('*').single();
    }catch{
      // Fallback: update departments rows that reference this company_code
      try{ await client.from('departments').update({ company_name: payload.name }).eq('company_code', payload.code); }catch{}
